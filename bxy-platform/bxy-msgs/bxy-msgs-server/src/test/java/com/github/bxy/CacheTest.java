...
import com.github.bxy.cache.repository.CacheRepository;
...
class CacTest {
    ...
    @Cacheable(value = "bxy", keyGenerator = "keyGenerator")
    public User bxy() {
        return User.builder()
                .account("bxy").name("张三李四!@#$%^&*()_123")
                .id(6079967614237410571L)
                .sex(Sex.M)
                .createTime(LocalDateTime.now())
                .build();
...
public class CacheTest {
    ...
    public void testCache() {
        ...
        cacheRepository.setExpire("bxy", user, 3);

        String wz = cacheRepository.get("wz");
        User user2 = cacheRepository.get("bxy");
        System.out.println(wz);
        System.out.println(user2);
        System.out.println(cacheRepository.del("wz2"));
        User user3 = cacheRepository.get("bxy");
